...
\begin{longtable}{ | p{2cm} | p{4cm} | p{5cm} | p{4cm} | } 
    ...
    \hline
        1.1 & 
        The processor should implement a RISC (Reduced Instruction Set Computer) design philosophy. 
        & A RISC instruction set allows more focus to be placed on the software side (the domain of this NEA) and results in faster and more efficient execution of it's instructions resulting in strong performance benefits, especially when a user isn't as familiar with the instruction set of the system. (Client Interview 1.1) 
        & \\
  \hline
        1.2 & 
        The processor should use a Harvard computer architecture where data and instructions are stored in seperate memories. & 
        Data and instructions can be fetched during the same processor cycle, so execution is more time efficient  and emulation easier (University of Washington MIPS Computer 1.)
        & \\
  \hline
        1.3 & 
        My assembly language should support 3 address operands and hide the need to address data and instructions from different memories.& 
        3 address operands is the programming standard for both arm and x86, pre-established instruction sets programemrs are already familiar with. (Client Interview 1.2)
        & 
  ...
  \hline
        1.4 & 
        My assembler should be able to calculate offsets of branch instructions from the position of labels within the source code. & 
        Labels provide a necessary convenience for programmers implement branching, selective statements, or functions without having to manually calculate memory offests. (The Hack Computer, The University of Washington MIPS Computer, Client Interview) & 
        My assembler should be able to replace all occurences of an ASCII label in the assembly source code with calculated offests from the current instruction to that label.\\
  \hline
        1.5 & 
        My assembler should provide a number of macro instructions that perform common tasks (e.g. pushing an address to the stack) that are not otherwise specified in the ISA. &
    ...
    \hline
        1.6 & 
        My pocessor should have the following internal registers: 16 general purpose registers, a Delay timer, Program Counter, and Stack Pointer.  &
    ...
    \hline
        1.7 & 
        My CPU should operate with a word length of 16-bits. &
        A 16-bit word length allow more bits to be processed in a single cycle than an 8-bit system, and allows 16-bit instructions to be fetched within a single cycle improving performance. 16-bits can also represent numbers of a greater magnitude or precision using floating point representation than 8-bits mitigating the prevelance of overflow errors. (Client Interview 1.4) &
    ...
    \hline
        2.1 & 
        The syntax of my language should follow C-like standards with semi colons and braces rather than relying on indentation. &
        This permits easier compilation and more flexibility when formatting code, as well as reprorting errors such as an unterminated brace at compile time (unlike incorrect indentation which may not be detected until debugging) (Client Interview 2.2) & 
    ...
    \hline
        ...
        A type system results in robust programs with type-errors thrown at compile time rather than during execution, meaning errors like accidentally reassigning the type of a variable will be detected by the compiler and programs are easier to debug. Furthermore compilation becomes easier with statically typed variables as their size in memory is predetermined. (Monkey.2, Jack.4, Client Interview 2.5) & 
        My syntax should have support for signed and unsigned integers, strings, and integers of different sizes. The \texttt{let} keyword should be used when declaring a variable and require the type to be specified alongside its identifier. \\
    \hline
\end{longtable}
\newpage